...
st.title("Log Masuk Penyelia Industri")
try:
...
if not st.session_state.auth:
    ...
    if submitted:
        ...
        if not user or user["role_name"]!="ind_sv":
    ...
    with get_conn() as conn:
        ...
        assigned = one(conn, "SELECT COUNT(1) FROM supervision WHERE ind_supervisor_id=?", (user['user_id'],))
        reported = one(conn, "SELECT COUNT(1) FROM reporting_in r JOIN supervision s ON r.student_id=s.student_id WHERE s.ind_supervisor_id=?", (user['user_id'],))
        bli05_done = one(conn, "SELECT COUNT(1) FROM bli05_industry WHERE ind_supervisor_id=?", (user['user_id'],))
    c1,c2,c3 = st.columns(3)
    c1.metric("Sesi", tlabel); c2.metric("Pelajar di bawah anda", f"{assigned}"); c3.metric("Lapor Diri (BLI-04)", f"{reported}")
    c4,_ = st.columns(2)
    c4.metric("BLI-05 dihantar", f"{bli05_done}")
    ...
    with get_conn() as conn:
        ...
        df_stu = pd.read_sql_query(
            "SELECT u.user_id, u.full_name, u.student_id FROM supervision s JOIN users u ON u.user_id=s.student_id WHERE s.term_id=? AND s.ind_supervisor_id=? ORDER BY u.full_name",
    ...
    if df_stu.empty:
        ...
        with get_conn() as conn:
            df_logs = pd.read_sql_query(
                "SELECT log_id, entry_date, title, activities, outcomes, hours, ind_comment FROM logbook WHERE student_id=? AND term_id=? ORDER BY entry_date DESC",
        ...
        if df_logs.empty:
            ...
            for i, row in df_logs.iterrows():
                with st.expander(f"Entri {row['entry_date']} — {row['title'] or '-'}"):
                    ...
                    new_c = st.text_area("Komen (Industri)", value=row["ind_comment"] or "", key=f"ind_c_{row['log_id']}")
                    if st.button("Simpan Komen (Industri)", key=f"btn_ind_{row['log_id']}"):
                        with get_conn() as conn:
                            cur = conn.cursor()
                            cur.execute("UPDATE logbook SET ind_comment=?, ind_commented_by=?, ind_commented_at=datetime('now') WHERE log_id=?", (new_c, user['user_id'], int(row["log_id"])))
                            conn.commit()
                        st.success("Komen industri disimpan."); st.rerun()
